...
void QF_init(void)
{
	...
	pthread_mutex_init(&QF_InitMutex, NULL);

	pthread_mutex_lock(&QF_InitMutex);
}
...
int_t QF_run(void)
{
	...
	pthread_mutex_unlock(&QF_InitMutex);
	...
	QF_onCleanup();
	pthread_mutex_destroy(&QF_InitMutex);
